...
benchinit() {
	...
	if  [ ! -e '/usr/bin/fio' ]; then
	        echo " Installing fio ..."
	            if [ "${release}" == "centos" ]; then
	                ...
	                apt-get -y install fio > /dev/null 2>&1
	            fi
		echo -ne "\e[1A"; echo -ne "\e[0K\r"
	fi
	...
	if  [ ! -e 'speedtest' ]; then
		...
		wget --no-check-certificate $BEST_CDN/repo/bench/speedtest --timeout=120 > /dev/null 2>&1
		echo -ne "\e[1A"; echo -ne "\e[0K\r"
	fi
...
speed_test(){
	if [[ $1 == '' ]]; then
		temp=$($PWD/speedtest -p no 2>&1)
		is_down=$(echo "$temp" | grep 'Download')
		result_speed=$(echo "$temp" | awk -F ':' '/URL/{print $2":"$3}')
		if [[ ${is_down} ]]; then
	        local REDownload=$(echo "$temp" | awk -F '[:(]' '/Download/{print $2}' | xargs)
	        local reupload=$(echo "$temp" | awk -F '[:(]' '/Upload/{print $2}' | xargs)
	        local relatency=$(echo "$temp" | awk -F '[:(]' '/Latency/{print $2}' | xargs)
		...
		temp=$($PWD/speedtest -s $1 -p no 2>&1)
		is_down=$(echo "$temp" | grep 'Download') 
		if [[ ${is_down} ]]; then
	        local REDownload=$(echo "$temp" | awk -F '[:(]' '/Download/{print $2}' | xargs)
	        local reupload=$(echo "$temp" | awk -F '[:(]' '/Upload/{print $2}' | xargs)
...
print_speedtest_europe() {
	...
	printf "%-75s\n" "-" | sed 's/\s/-/g' | tee -a $log
        speed_test '' 'Nearby                          '
	printf "%-75s\n" "-" | sed 's/\s/-/g' | tee -a $log
	speed_test '44081' 'Germany, Frankfurt am Main (23M GmbH)           ' 'http://speedtest.23m.com'
	speed_test '65026' 'Netherlands, Dronten (MIRHosting)           ' 'http://speedtest.nl1.mirhosting.net'
	speed_test '61933' 'France, Paris (Scaleway)           ' 'http://st1.scaleway.com'
	speed_test '31183' 'United Kingdom, London (RETN)           ' 'http://speedtest-lon.retn.net'
	speed_test '21429' 'Czechia, Prague (10G-CZNET.CZ)           ' 'http://speedtest.cznet.cz'
	speed_test '63744' 'Moldova, Chisinau (Moldcell)           ' 'http://speedtest100.moldcell.md'
	speed_test '14928' 'Finland, Helsinki (Telia)           ' 'http://speedtest.extra.telia.fi'
	speed_test '32927' 'Poland, Warsaw (RETN)           ' 'http://speedtest-waw.retn.net'
...
print_speedtest_ru() {
	...
	printf "%-75s\n" "-" | sed 's/\s/-/g' | tee -a $log
        speed_test '' 'Nearby                        '
	printf "%-75s\n" "-" | sed 's/\s/-/g' | tee -a $log
	speed_test '23499' 'Russia, Moscow (Марьино.net)     ' 'http://speedtest.maryno.net'
	speed_test '44487' 'Russia, Moscow (VDSina)      ' 'http://speedtest-ru.vdsina.ru'
	speed_test '46685' 'Russia, Moscow (fdcservers.net)         ' 'http://lg-dme.fdcservers.net'
	speed_test '34652' 'Russia, Saint Petersburg (G&G Telecom)  ' 'http://speedtest.gigt.ru'
	speed_test '65235' 'Russia, Saint Petersburg (Evro Telecom) ' 'http://speedtest.evrotm.ru'
	speed_test '46231' 'Russia, Novosibirsk (SibSvst Ltd)       ' 'http://speedtest.sibsvst.ru'
	speed_test '27144' 'Russia, Kemerovo (Yacolo)       ' 'http://speedtest.yacolo.com'
	speed_test '46700' 'Russia, Khabarovsk (fdcservers.net)       ' 'http://lg-khv.fdcservers.net'
...
geekbench_req() {
	if ! command -v ldd &> /dev/null; then
		echo -e " \e[31mError: 'ldd' command not found. GLIBC might not be installed or in your PATH.\e[0m"
		exit 1
	fi
	# Get GLIBC version using ldd
	requiredver="2.27"
	echo -n " checking version glib: "
	if ldd --version | sed 's/.* //;q' | awk -F. '{ if ($1 > 2 || ($1 == 2 && $2 >= 27)) { exit 0 } else {exit 1} }' ; then
		echo -e "\e[32mOK >=2.27\e[0m";
		geekbench6;
	else
		echo -e "\e[33mOutdated <2.27";
		echo -e " GLIBC Less than ${requiredver}. Geekbench5 instead v6.\e[0m"
		geekbench5;
	fi
...
ip_info4(){
	...
	as_tmp=$(python3 tools.py geoip ASN)
	asn=$(echo $as_tmp | awk -F ' ' '{print $1}')
	org=$(python3 tools.py geoip ISP)
	country=$(python3 tools.py geoip Country)
	city=$(python3 tools.py geoip City)
	countryCode=$(python3 tools.py geoip Short_name)
	region=$(python3 tools.py geoip City)

	echo -e " ASN & ISP    : AS$asn, $isp" | tee -a $log
	echo -e " Organization : $org" | tee -a $log
	echo -e " Location     : $city, $country / $countryCode" | tee -a $log
	echo -e " Region       : $region" | tee -a $log
...
io_test_fio() {
	readtest = $( fio --randrepeat=1 --ioengine=libaio --direct=1 --gtod_reduce=1 --name=test --filename=testio --bs=4k --iodepth=32 --size=1G --readwrite=randread )
	writetest = $( fio --randrepeat=1 --ioengine=libaio --direct=1 --gtod_reduce=1 --name=test --filename=testio --bs=4k --iodepth=32 --size=1G --readwrite=randwrite )
	read_iops=$(grep -m1 "read:" "$readtest" | sed -E 's/.*IOPS=([^,]+).*/\1/')
	write_iops=$(grep -m1 "write:" "$writetest" | sed -E 's/.*IOPS=([^,]+).*/\1/')

	read_bw=$(grep -m1 "read:" "$readtest" | sed -E 's/.*BW=([^ ]+).*/\1/')
	write_bw=$(grep -m1 "write:" "$writetest" | sed -E 's/.*BW=([^ ]+).*/\1/')

	echo "Fio READ  IOPS: $read_iops"
	echo "Fio READ  BW:   $read_bw MB/s"
	echo "Fio WRITE IOPS: $write_iops"
	echo "Fio WRITE BW:   $write_bw MB/s"
}
...
write_io() {
	...
	if [[ $writemb != "1" ]]; then
		...
		read_iops=$(grep -m1 "read:" "readtest.txt" | sed -E 's/.*IOPS=([^,]+).*/\1/')
		read_avg_iops=$(grep -m1 "^ *iops" readtest.txt | \
					sed -E 's/.*avg=([0-9.]+).*/\1/')
		read_bw=$(grep -m1 "read:" "readtest.txt" | sed -E 's/.*BW=([^ ]+).*/\1/')
		
		echo -e "$read_avg_iops | $read_bw MB/s" | tee -a $log
		...
		write_avg_iops=$(grep -m1 "^ *iops" writetest.txt | \
					sed -E 's/.*avg=([0-9.]+).*/\1/')
		echo -e "$write_avg_iops | $write_bw MB/s" | tee -a $log
		rm -f testio;
		rm -f writetest.txt;
		rm -f readtest.txt;
		echo "" | tee -a $log
...
sharetest() {
	...
	log_preupload
	board_name=$( dmidecode -t 4 | grep ID | sed 's/.*ID://;s/ //g' )
	logpost=$( cat $log )
	logpost2=$(urlencode "$logpost")
	CURL_DATA='clbin='$logpost2'&board='$board_name;
	case $1 in
	'ubuntu')
...
case $1 in
	...
	'europe'|'-europe'|'--europe'|'eu'|'-eu'|'--eu'|'Europe'|'-Europe'|'--Europe' )
		europe_bench;;
	'russia'|'-russia'|'--russia'|'ru'|'-ru'|'--ru'|'Russia'|'-Russia'|'--Russia' )
		ru_bench;;
